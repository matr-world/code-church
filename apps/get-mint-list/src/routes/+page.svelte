<script lang="ts">
    import validateAddress from "src/lib/util/validate-address";
    ...
    $: maybeSorted = finishedGeneratingSnapshot ? snapshotSuccess.filter((holder) => holder).sort((a, b) => a.localeCompare(b)) : snapshotSuccess;
    ...
    const downloadSnapshot = () => {
        downloadString(result, "text/plain", `snapshot-${firstCreatorAddress}.txt`);
    };
    ...
    const generateSnapshot = async () => {
        failedToGenerateSnapshot = false;
